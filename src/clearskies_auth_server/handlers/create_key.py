...
class CreateKey(Base):
    ...
    def handle(self, input_output):
        ...
        private_keys[key_id] = {**json.loads(key.export_private()), 'issue_date': self._datetime.datetime.now(self._datetime.timezone.utc).isoformat()}
        public_keys[key_id] = {**json.loads(key.export_public()), 'issue_date': self._datetime.datetime.now(self._datetime.timezone.utc).isoformat()}
