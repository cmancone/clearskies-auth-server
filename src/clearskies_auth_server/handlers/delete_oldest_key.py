...
class DeleteOldestKey(Base):
    ...
    def handle(self, input_output):
        ...
        key_id = min(private_keys,
                     key=lambda key_id: private_keys[key_id]['issue_date'])

        private_keys = {
            key: value
            for (key, value) in private_keys.items() if key != key_id
        }
        public_keys = {
            key: value
            for (key, value) in public_keys.items() if key != key_id
        }
